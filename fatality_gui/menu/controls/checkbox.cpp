...
{
	_u(key, m_value);
	render::get().bordered_box(m_bounds.x, m_bounds.y, CHECKBOX_HEIGHT, CHECKBOX_HEIGHT, m_hovered ? D3DCOLOR_RGBA(154, 10, 79, 250) : D3DCOLOR_RGBA(60, 53, 93, 255));
	...
	render::get().text(Vector2D(m_bounds.x + CHECKBOX_HEIGHT + 4, m_bounds.y + (CHECKBOX_HEIGHT / 2) - (render::get().get_text_size(name, fonts::controlfont).y / 2) - 1),
		name, Color(255, 255, 255, 255), fonts::controlfont, c_font::left_aligned);
	...
	if (!g_cfg[key]->get<bool>() && anim_progress <= 0.f)
		//render::get().filled_box(m_bounds.x + 1, m_bounds.y + 1, CHECKBOX_HEIGHT - 2, CHECKBOX_HEIGHT - 2, D3DCOLOR_RGBA(255, 255, 255, (int)(60 * hover_progress)));
		render::get().filled_box(m_bounds.x + 1, m_bounds.y + 1, CHECKBOX_HEIGHT - 2, CHECKBOX_HEIGHT - 2, D3DCOLOR_RGBA(60, 53, 93, (int)(60 * hover_progress)));

	if (g_cfg[key]->get<bool>() && anim_progress >= 1.f)
	{
		D3DCOLOR col = D3DCOLOR_RGBA(235, 5, 90, 100);
