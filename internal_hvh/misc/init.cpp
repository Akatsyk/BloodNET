...
void __forceinline __cdecl init::get_interfaces()
{
	...
	_(VEngineEffects001, "VEngineEffects001" );
	g_pEffects = util::get_interface<IVEffects>( engine, VEngineEffects001);
...
void init::get_materials()
{
	...
	_(cstm_visible_gl, "cstm_visible_gl");
	chams::get().add_material(material_index_t::MAT_VISIBLE_GLOW, g_pMaterialSystem->FindMaterial(cstm_visible_gl, tex_group));
	_(cstm_hid, "cstm_hid");
	...
	_(cstm_hid_gl, "cstm_hid_gl");
	chams::get().add_material(material_index_t::MAT_HID_GLOW , g_pMaterialSystem->FindMaterial(cstm_hid_gl, tex_group));
...
void init::hooks()
{
	...
	predict = new c_hook( g_pPrediction );
	...
	orig_run_command = predict->apply<run_command_fn>(19, run_command);
