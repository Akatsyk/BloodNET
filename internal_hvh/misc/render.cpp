...
void render::rect_filled_diamond_gradient( Vector2D from, Vector2D size, Color color1, Color color2 ) const
{
	...
	dev->SetTexture(0, nullptr);
	dev->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 4, &vert, sizeof vertex);
}
...
void render::draw_arc(Vector2D center, int radius, int start_angle, int percent, int thickness, Color color)
{
...
void render::circle_filled_new(Vector2D center, int points, int radius, Color color)
{
	const auto col = color.direct();
	...
	{
		float eeks = radius * temppointsx[i] + center.x;
		float why = radius * temppointsy[i] + center.y;
	...
	dev->SetTexture(0, nullptr);
	dev->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, points, vertices.data(), sizeof vertex);
}

void render::circle_filled_3d(Vector center, int radius, Color color)
{
	float step = M_PI * 2.0f / points;
	for (float a = 0; a < M_PI * 2.0f; a += step) {
		Vector start(radius * cosf(a) + center.x, radius * sinf(a) + center.y, center.z);
