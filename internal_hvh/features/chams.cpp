...
bool chams::draw_ch(IMatRenderContext* context, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* pCustomBoneToWorld)
{
	...
	if (vars.visuals.chams.hidden.get<bool>() && player != g_pLocalPlayer)
	{
		if (player->is_enemy())
		{
			...
			glow = type == MAT_HID_GLOW || type == MAT_VISIBLE_GLOW;
		...
		if (glow)
		{
			g_pRenderView->SetColorModulation(col.Base());
			g_pRenderView->SetBlend(col.aBase());
			g_pModelRender->ForcedMaterialOverride(materials[material_index_t::MAT_HID_FLAT]);
	...
	{
		g_pRenderView->SetColorModulation(col.Base());
		g_pRenderView->SetBlend(/*col.aBase() */ 1.f - transparency * 0.01f);
		g_pModelRender->ForcedMaterialOverride(materials[material_index_t::MAT_VISIBLE_FLAT]);
		hooks::orig_draw_model_execute(g_pModelRender, context, state, info, pCustomBoneToWorld);
	...
	g_pModelRender->ForcedMaterialOverride(material);
	if(!glow) hooks::orig_draw_model_execute(g_pModelRender, context, state, info, pCustomBoneToWorld);
	return true;
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
	...
	_(cstm_visible_gl, "csgo\\materials\\cstm_visible_gl.vmt");
	__(cstm_visible_gl_s, R"#("VertexLitGeneric"
	{
		"$additive" "1"
	...
	std::ofstream(cstm_visible_gl) << cstm_visible_gl_s;


	_(cstm_hid_gl, "csgo\\materials\\cstm_hid_gl.vmt");
	__(cstm_hid_gl_s, R"#("VertexLitGeneric"
{
	"$additive" "1"
	"$envmap" "models/effects/cube_white"
	"$envmaptint" "[1 1 1]"
	...
	std::ofstream(cstm_hid_gl) << cstm_hid_gl_s;
...
chams::~chams()
{
	...
	std::remove(cstm_hid);
	__(cstm_visible_gl, "csgo\\materials\\cstm_visible_gl.vmt");
	std::remove(cstm_visible_gl);
	__(cstm_hid_gl, "csgo\\materials\\cstm_hid_gl.vmt");
	std::remove(cstm_hid_gl);
