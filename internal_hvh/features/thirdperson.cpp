...
void thirdperson::run() const
{
	...
	if (g_pLocalPlayer->get_alive() && vars.misc.thirdperson.get<bool>())
	{
		...
		// start the animation.
		progress += g_pGlobals->frametime * 5.f + (progress / 100);

		// clamp. 
		math::get().clamp(progress, 0.f, 1.f);

		g_pInput->m_fCameraInThirdPerson = true;
		g_pInput->m_vecCameraOffset = Vector(view_angles.x, view_angles.y, vars.misc.thirdperson_dist.get<float>() * progress);
	...
	else if (g_pInput->m_fCameraInThirdPerson)
	{
		...
		// clamp.
		math::get().clamp(progress, 0.f, 1.f);
		g_pInput->m_vecCameraOffset.z = vars.misc.thirdperson_dist.get<float>() * progress;
