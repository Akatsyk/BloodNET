...
{
	dormancy_manager::get().sound_dormancy();
	players_get();
...
void visuals::draw_scope()
{
	...
	int gOffy = h / 2 / 2;
	render::get().rect_filled_linear_gradient(Vector2D(w / 2 - gOffy, h / 2), Vector2D(gOffy - 8, 1), Color(255, 255, 255, 0), Color(255, 255, 255, alpha), true); // LEFT
	render::get().rect_filled_linear_gradient(Vector2D(w / 2 + 8, h / 2), Vector2D(gOffy, 1), Color(255, 255, 255, alpha), Color(255, 255, 255, 0), true); // RIGTH

	render::get().rect_filled_linear_gradient(Vector2D(w / 2, h / 2 - gOffy), Vector2D(1, gOffy - 8), Color(255, 255, 255, 0), Color(255, 255, 255, alpha), false); // UP
	render::get().rect_filled_linear_gradient(Vector2D(w / 2, h / 2 + 8), Vector2D(1, gOffy), Color(255, 255, 255, alpha), Color(255, 255, 255, 0), false); // DOWN
}
...
void visuals::watermark()
{
	...
	if ((g_pGlobals->tickcount - old_tick_count) > 50) {
		fps = static_cast<int>(1.f / g_pGlobals->frametime);
		old_tick_count = g_pGlobals->tickcount;
	}
	...
	std::string incoming = g_pLocalPlayer ? std::to_string((int)(net_channel->GetLatency(FLOW_INCOMING) * 1000)) : "0";
	std::string outgoing = g_pLocalPlayer ? std::to_string((int)(net_channel->GetLatency(FLOW_OUTGOING) * 1000)) : "0";

	ss << "bloodnet" << " | incoming: " << incoming.c_str() << "ms" << " | outgoing: " << outgoing.c_str() << "ms" << " | " << GetTimeString() << "";

	int x, y, w, h;

	auto textsize = render::get().get_text_size(ss.str().c_str(), fonts::esp_small);

	render::get().rect_filled(Vector2D(screen_x - (textsize.x + 20), 6), Vector2D(textsize.x + 20, 18), Color(60, 53, 93, 255));
	render::get().rect(Vector2D(screen_x - (textsize.x + 20), 6), Vector2D(textsize.x + 20, 18), Color(10, 10, 10, 255));

	render::get().text(Vector2D(screen_x - (textsize.x + 10), 9), ss.str().c_str(), Color(255, 255, 255, 200), fonts::esp_small);
}
...
void visuals::aa_indicator()
{
	...
	render::get().text( Vector2D(10, size.Height / 2.f + 20), text, get_col( vars.aa.indicator_col.get<uintptr_t>() ), fonts::lby, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow );
...
void visuals::lby_indicator()
{
	if (!vars.aa.regular.lby.get<bool>() || !g_pEngine->IsInGame() || !g_pLocalPlayer->get_alive())
		return;

	return;

	//const auto playerRecord = &player_log::get().get_log(g_pLocalPlayer->EntIndex());

	//float breaking_lby_fraction = fabs(math::get().normalize_float(prediction::get().get_unpred_eyepos().y - g_pLocalPlayer->get_lby())) / 180.f;

	//Color color_lby = Color(255, 0, 0, 255);

	//if (breaking_lby_fraction > 35.f || playerRecord->nextBodyUpdate <= 0.f)
	//	color_lby = Color(0, 255, 0, 255);

	//const auto size = render::get().get_dimensions();
	//_(lowerbody, "LBY");
	//render::get().text(Vector2D(10, size.Height / 2.f + 40), lowerbody, color_lby, fonts::lby, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow);

	////if ((prediction::get().get_unpred_vel().Length() < 0.1f || (GetAsyncKeyState(vars.key.fakewalk.get<int>()) && vars.aa.fakewalk.get<bool>())))
	//if (playerRecord->m_bRunningTimer)
	//{
	//	render::get().filled_box(10, size.Height / 2.f + 55, 41, 2, Color(0, 0, 0, 170));

	//	float progress = playerRecord->nextBodyUpdate / 1.1f;
	//	progress = std::clamp(progress, 0.f, 1.1f);

	//	if (progress < 1.1f)
	//		render::get().filled_box(10, size.Height / 2.f + 55, 40 * progress, 1, Color((1.f - breaking_lby_fraction) * 255.f, breaking_lby_fraction * 255.f, 0));//color_lby);
	//}
}

// meme...
void visuals::antiaim_arrows()
{
	...
	math::get().angle_vectors(QAngle(0, antiaim::get().lineLBY, 0), &forward);
	src3D = g_pLocalPlayer->get_origin();
	dst3D = src3D + (forward * 35.f); //replace 50 with the length you want the line to have

	ray.Init(src3D, dst3D);

	g_pTrace->TraceRay(ray, 0, &filter, &tr);

	if (!math::get().world_to_screen(src3D, src) || !math::get().world_to_screen(tr.endpos, dst))
		return;

	_(lowerbody, "LBY");
	render::get().line(Vector2D(src.x, src.y), Vector2D(dst.x, dst.y), Color(255, 255, 0, 255));
	render::get().text(Vector2D(dst.x, dst.y), lowerbody, Color(255, 165.f, 0, 255), fonts::controlfont, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow);

	math::get().angle_vectors(QAngle(0, antiaim::get().lineRealAngle, 0), &forward);
	dst3D = src3D + (forward * 35.f); //replace 50 with the length you want the line to have
	...
	_(real, "REAL");
	render::get().line(Vector2D(src.x, src.y), Vector2D(dst.x, dst.y), Color(0, 70, 230, 255));
	render::get().text(Vector2D(dst.x, dst.y), real, Color(0, 70.f, 230, 255), fonts::controlfont, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow);

	math::get().angle_vectors(QAngle(0, antiaim::get().lineFakeAngle, 0), &forward);
	dst3D = src3D + (forward * 35.f); //replace 50 with the length you want the line to have
	...
	_(fake, "FAKE");
	render::get().line(Vector2D(src.x, src.y), Vector2D(dst.x, dst.y), Color(255, 0, 255, 255));
	render::get().text(Vector2D(dst.x, dst.y), fake, Color(255, 0.f, 0, 255), fonts::controlfont, c_font::left_aligned | c_font::centered_y | c_font::drop_shadow);
}
