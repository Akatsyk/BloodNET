...
void animations::update_player_animations( lag_record_t* record, C_CSPlayer* m_player )
{
	...
	if ( record->m_lag > 1 && !bot )
	{
		...
		if ( log->record.size() >= 2 )
		{
			...
			if ( previous && !previous->m_dormant )
			{
				...
				float change = (duck / time) * g_pGlobals->interval_per_tick;

				// fix crouching players.
				m_player->get_duck_amt() = previous->m_duck + change;

				if (!record->m_fake_walk) 
				{
					// fix the velocity till the moment of animation.
					Vector velo = record->m_velocity - previous->m_velocity;

					// accel per tick.
					Vector accel = (velo / time) * g_pGlobals->interval_per_tick;
	...
	//const auto simulation_tick_delta = time_to_ticks(m_player->get_simtime() - m_player->get_oldsimtime());
	//const auto nci = g_pEngine->GetNetChannelInfo();
	//const auto delta_ticks = (std::clamp(time_to_ticks(nci->GetLatency(FLOW_INCOMING) + nci->GetLatency(FLOW_INCOMING)) + g_pGlobals->tickcount - time_to_ticks(m_player->get_simtime() + lagcomp::get_lerp_time()), 0, 100)) - simulation_tick_delta;
	...
	//	while (ticks_left > 0)
	//	{
	//		auto data_origin = m_player->get_origin();
	//		auto data_velocity = m_player->get_velocity();
	//		auto data_flags = m_player->get_flags();

	//		lagcomp::extrapolate(m_player, data_origin, data_velocity, data_flags, in_air);

	//		m_player->get_simtime() += g_pGlobals->interval_per_tick;
	//		m_player->get_origin() = data_origin;
	//		m_player->get_velocity() = data_velocity;
	//		m_player->get_flags() = data_flags;
	//		--ticks_left;
	//	}
	//}
