...
void resolver::resolve(C_CSPlayer* player, lag_record_t* record)
{
	...
	const auto velocity = record->m_anim_velocity.Length();

	if (velocity > 0.1f && !record->m_fake_walk)
	{
		if (is_spin(&log))
		{
	...
	if (log.m_iMode == RMODE_WALL)
	{
		if (!wall_detect(&log, record, record->m_eye_angles.y))
			log.m_iMode = RMODE_DELTA;//RMODE_MOVING;
	}
...
void resolver::collect_wall_detect(const ClientFrameStage_t stage)
{
	...
	for (auto i = 1; i < g_pGlobals->maxClients; i++)
	{
		...
		if (log.m_iMode == RMODE_WALL)
		{
			...
			auto left_eye_pos = eye_pos + (left_dir * 16.f); // was 16.f
...
void resolver::extrpolate_players(const ClientFrameStage_t stage) const
{
	...
	for (auto i = 1; i < g_pGlobals->maxClients; i++)
	{
		...
		if (simulation_tick_delta > 17 && simulation_tick_delta < 2)
			return;
	...
	if (lby_update_delta > 0.1875f && choked_ticks)
	{
		...
		if (onground && speed <= 0.1 && lby_moving_update_delta <= 1.33f)
		{
			if (log.m_nShots > 0)
			{
				...
				log.m_flLastMovingLowerBodyYawTarget = math::normalize_float(log.m_flLowerBodyYawTarget + brutelist_ns[index]);
			}
				...
				if (ongroundrecord)
				{
					...
					else
					{
						...
						if (log.m_nShots > 0)
						{
							...
							average_lby = log.m_flLowerBodyYawTarget + brutelist_ns[index];
						}
...
void resolver::listener(IGameEvent* game_event)
{
	...
	if (vars.misc.impacts.get<bool>())
		g_pDebugOverlay->AddBoxOverlay(pos, Vector(-2, -2, -2), Vector(2, 2, 2), QAngle(0, 0, 0), 0, 0, 155, 127, 6);
...
void resolver::calc_missed_shots(shot_t* shot)
{
	...
	_(bloodnet, "[ BloodNET ] ");
	_(miss, "MISS ");
	_(unknwn, "[unknown reason]\n");
	_(rslvr, "[bad resolve] ");
	_(mode, "MODE ");
	_(sprd, "[spread]\n");
	//_(reason, "[none]\n");
	const char* reason_miss = "[miss]\n";
	switch (log->m_iMode)
	{
	case RMODE_DELTA:
		break;
	...
	if (shot->hit || nospread->get_bool())
	{
		if (vars.aim.resolver.get<bool>())
		{
			...
			else
			{
				...
				util::print_dev_console(true, Color(51, 171, 249, 255), mode);
				util::print_dev_console(true, Color(255, 255, 255, 255), reason_miss);
			}
		}
	}
	else
